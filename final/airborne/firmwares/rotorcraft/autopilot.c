...
void autopilot_periodic(void) {
	...
	if (autopilot_mode == AP_MODE_KILL || autopilot_mode == AP_MODE_FAILSAFE) {
		...
		SetRotorcraftCommands(stabilization_cmd, autopilot_in_flight,
				autopilot_motors_on);
	}
...
void autopilot_set_mode(uint8_t new_autopilot_mode) {
	...
	if (new_autopilot_mode != autopilot_mode) {
		...
		case AP_MODE_NAV:
			...
			break;
		case AP_MODE_OPT_TRAJ:
			guidance_h_mode_changed(GUIDANCE_H_MODE_OPT_TRAJ);
			break;
		default:
			break;
		}
		...
		case AP_MODE_NAV:
			...
			break;
		case AP_MODE_OPT_TRAJ:
			guidance_v_mode_changed(GUIDANCE_V_MODE_OPT_TRAJ);
			break;
		default:
			break;
		}
