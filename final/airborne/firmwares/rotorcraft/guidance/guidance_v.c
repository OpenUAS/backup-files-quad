...
__attribute__ ((always_inline)) static inline void run_hover_loop(
		bool_t in_flight);
...
void guidance_v_run(bool_t in_flight) {
	...
	if (in_flight) {
		gv_adapt_run(stateGetAccelNed_i()->z, stabilization_cmd[COMMAND_THRUST],
				guidance_v_zd_ref);
	}
	...
	case GUIDANCE_V_MODE_OPT_TRAJ:
			stabilization_cmd[COMMAND_THRUST] = ipopt_cmd[0]/ipopt_cmd[2]*MAX_PPRZ;
			break;
