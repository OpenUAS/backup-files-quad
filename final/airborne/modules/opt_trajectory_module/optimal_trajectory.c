...
#include "paparazzi.h"
/*Array of control commands for paparazzi*/
float ipopt_cmd[3];
/*Variables that will store current state values*/
struct FloatEulers* currAng;
struct EnuCoor_f* currCoord;
struct EnuCoor_f* currSpeed;
...
struct MyUserData {
	Number g_offset[2]; /* Structure required by IPOPT C interface however not used for our problem.*/
};
...
int ipoptMain(void) {
	...
	/*Set upper and lower boundaries for variables*/
	for (i = 0; i < N_; i++) {
		x_L[xP + i] = -2e19;
		x_U[xP + i] = 2e19;
	...
	/*We use current state values as initial values*/
	currAng = stateGetNedToBodyEulers_f();
	currCoord = stateGetPositionEnu_f();
	currSpeed = stateGetSpeedEnu_f();
	...
	/*Pass the control values thrust and theta for the next state. We pass maxthrust in order to use
	 * percentage value of maximum thrust in paparazzi*/
	ipopt_cmd[0] = x[u1P+1];
	ipopt_cmd[1] = x[thetaP+1];
	ipopt_cmd[2] = maxthrust;
...
Bool eval_g(Index n, Number* x, Bool new_x, Index m, Number* g,
	...
	g[0] = x[xP] - currCoord->y;
	g[1] = x[zP] - currCoord->z;
	g[2] = x[vxP] - currSpeed->y;
